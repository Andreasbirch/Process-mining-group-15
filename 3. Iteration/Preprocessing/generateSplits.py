...
def generateSplits():
    ...
    files = os.listdir("3. Iteration/Splits")
    # Read each file in Splits directory
    for file in files:
        df = pd.read_csv('3. Iteration/Splits/'+ str(file))
        ...
        if len(list) > 1:
            ...
            newDf.to_csv("3. Iteration/Test-Train/Train/" + file, index=False)
            ...
            # Create a csv file for each connector stored in folder Splits
            newDf2.to_csv("3. Iteration/Test-Train/Test/" + file, index=False)
